  ...
  const scrapeEachLink = (arrayOfLinks) => {
    return bluebirdPromise.map(overviewURL, (showDetailsURL) => {
      ...
      .then(window => {
        ...
        cost === null ? cost = scrapedCost : cost = cost[1];
        ...
        const scrapedArtistSummary = window.document.querySelector('.ds-event-description-inner p').textContent;
        const artistSummary = scrapedArtistSummary;
